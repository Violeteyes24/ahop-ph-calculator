...
export async function confirmPayrollImportAction(batchId: string, formData: FormData): Promise<void> {
  ...
  const period = await prisma.$transaction(async (tx) => {
    ...
    for (const row of data.employees) {
      ...
      const taxable = row.taxable.toLowerCase() === "yes";
      ...
      await tx.attendanceEntry.upsert({
        ...
        update: {
          ...
          expectedWorkHoursPay: row.expectedWorkHoursPay,
          scheduledWorkDays: row.scheduledWorkDays,
          scheduledWorkDaysPay: row.scheduledWorkDaysPay,
          ...
          tardinessDeduction,
          aotMinutes: row.aotMinutes,
          aotPay: row.aotPay,
          extraOtPremium: row.extraOtPremium,
          regularHolidayHours: row.regularHolidayHours,
          regularHolidayPay: row.regularHolidayPay,
          specialHolidayHours: row.specialHolidayHours,
          specialHolidayPay: row.specialHolidayPay,
          totalHolidayPay: row.totalHolidayPay,
          coAhop: row.coAhop,
          totalAhop: row.totalAhop,
          withholdingTax: row.withholdingTax,
          sourceGrossIncome: row.grossIncome,
          loanDeductions,
          salaryAdjustments,
